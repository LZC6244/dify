        ...
        body={
          <div className='flex flex-col h-full'>
            ...
            <div className={cn(isEdit ? 'justify-between' : 'justify-end', 'mt-2 shrink-0 flex py-4 px-6 rounded-b-[10px] bg-gray-50 border-t border-black/5')} >
              {
                isEdit && (
                  <ButtonNormal onClick={onRemove} className='text-red-500 border-red-50 hover:border-red-500'>{t('common.operation.delete')}</ButtonNormal>
                )
              }
              <div className='flex space-x-2 '>
                <ButtonNormal onClick={onHide}>{t('common.operation.cancel')}</ButtonNormal>
                <Button className='h-8 py-1 px-4 text-[13px] text-white' variant='primary' onClick={handleSave}>{t('common.operation.save')}</Button>
              </div>
            </div>
          </div>
        }
