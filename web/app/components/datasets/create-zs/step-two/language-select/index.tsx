...
const LanguageSelect: FC<ILanguageSelectProps> = ({
  ...
  return (
    <Popover
      ...
      htmlContent={
        <div className='w-full py-1'>
          {languages.filter(language => language.supported).map(({ prompt_name, name }) => (
            <div
              key={prompt_name}
              className='py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 rounded-lg cursor-pointer text-[#9EADB9] text-sm'
      ...
      btnClassName={open => cn('!border-0 !px-0 !py-0 !bg-inherit !hover:bg-inherit', open ? 'text-[#5E3EFB]' : 'text-[#9EADB9]')}
