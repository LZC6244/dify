...
import { FieldInfo } from '@/app/components/datasets/documents/detail/metadata-zs'
...
const EmbeddingProcess: FC<Props> = ({ datasetId, batchId, documents = [], indexingType }) => {
  ...
  return (
    <>
      <div className='h-5 flex justify-between items-center mb-3 mt-6'>
      ...
      <div className='flex items-center gap-2 mt-10'>
        <Button className='w-fit px-[22px] py-[12px] text-[16px] leading-4 font-normal rounded-[20px]' variant='primary' onClick={navToDocumentList}>
          <span>{t('datasetCreation.stepThree.navTo')}</span>
          <ArrowRightIcon className='h-[14px] w-[14px] ml-2 stroke-current stroke-1' />
        </Button>
      </div>
    </>
  )
