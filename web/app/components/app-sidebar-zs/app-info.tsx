...
import AppIcon from '../base/app-icon-zs'
    ...
    >
      <div className='relative'>
        <PortalToFollowElemTrigger
          ...
          className='block'
        >
          <div className={cn('flex p-1 rounded-lg', open && 'bg-gray-100', isCurrentWorkspaceEditor && 'hover:bg-[#F3F2FF] cursor-pointer')}>
            <div className='relative shrink-0 mr-2'>
              <AppIcon size={expand ? 'large' : 'small'} className='!w-[40px] !h-[40px] !rounded-[20px] !overflow-hidden' icon={appDetail.icon} background={appDetail.icon_background} />
            </div>
            {expand && (
              <div className="grow w-0">
                <div className='flex justify-between items-center text-[16px] leading-4 text-[#000000] font-medium'>
                  <div className='truncate' title={appDetail.name}>{appDetail.name}</div>
                  {/* {isCurrentWorkspaceEditor && <RiArrowDownSLine className='shrink-0 ml-[2px] w-3 h-3 text-gray-500' />} */}
                </div>
                <div className='mt-2 flex items-center text-[#9EADB9] text-[12px] leading-3 gap-2'>
                  {appDetail.mode === 'advanced-chat' && (
                    <>
                      <div className='shrink-0 '>{t('app.types.chatbot').toUpperCase()}</div>
                      <div title={t('app.newApp.advanced') || ''} className=''>{t('app.newApp.advanced').toUpperCase()}</div>
                    </>
                  )}
                  {appDetail.mode === 'agent-chat' && (
                    <div className='shrink-0 '>{t('app.types.agent').toUpperCase()}</div>
                  )}
                  {appDetail.mode === 'chat' && (
                    <>
                      <div className='shrink-0 '>{t('app.types.chatbot').toUpperCase()}</div>
                      <div title={t('app.newApp.basic') || ''} className=''>{(t('app.newApp.basic').toUpperCase())}</div>
                    </>
                  )}
                  {appDetail.mode === 'completion' && (
                    <>
                      <div className='shrink-0 '>{t('app.types.completion').toUpperCase()}</div>
                      <div title={t('app.newApp.basic') || ''} className=''>{(t('app.newApp.basic').toUpperCase())}</div>
                    </>
                  )}
                  {appDetail.mode === 'workflow' && (
                    <div className='shrink-0 '>{t('app.types.workflow').toUpperCase()}</div>
                  )}
                </div>
              </div>
            )}
