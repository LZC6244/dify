  ...
  return (
    <div>
      <div className={cn(!isShowDetail && 'shadow-sm', !isShowDetail && 'inline-block', 'max-w-full overflow-x-auto bg-white rounded-md')}>
        ...
        >
          {!isFinished && (
            <RiLoader2Line className='w-[16px] h-[16px] text-gray-500 animate-spin shrink-0' />
          )}
          {isFinished && !isShowDetail && (
            <CheckCircle className='w-[16px] h-[16px] text-[#12B76A] shrink-0' />
          )}
          {isFinished && isShowDetail && (
            icon
          )}
          <span className='mx-1 !text-[14px] font-medium text-gray-500 shrink-0'>
            {t(`tools.thought.${isFinished ? 'used' : 'using'}`)}
          </span>
          <span
            className='!text-[14px] font-medium text-gray-700 truncate'
            title={toolLabel}
          >
            {toolLabel}
          </span>
          <RiArrowDownSLine
            className={cn(isShowDetail && 'rotate-180', 'ml-1 w-[16px] h-[16px] text-gray-500 select-none cursor-pointer shrink-0')}
          />
        </div>
        {isShowDetail && (
