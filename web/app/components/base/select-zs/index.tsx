  ...
  return (
    ...
    >
      <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)} className='w-full'>
        <div
          className={`
            flex items-center justify-between px-[18px] h-[42px] rounded-lg border-0 bg-[#F3F4F7] text-sm cursor-pointer 
          `}
        ...
        >
          {items.map((item: Item) => (
            <div
              key={item.value}
              className={`
                flex items-center justify-between px-[18px] h-9 cursor-pointer rounded-lg hover:bg-gray-100 text-gray-700
