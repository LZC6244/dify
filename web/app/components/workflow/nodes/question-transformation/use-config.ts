...
const useConfig = (id: string, payload: QuestionTransformationNodeType) => {
  ...
  const { inputs, setInputs } = useNodeCrud<QuestionTransformationNodeType>(id, payload)
  ...
  const {
    ...
    setRunInputData,
    runResult,
  } = useOneStepRun<QuestionTransformationNodeType>({
    id,
    data: inputs,
    defaultRunInputData: {
      query: '',
