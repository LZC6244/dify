...
import AutomaticBtn from '@/app/components/app/configuration/configZS/automatic-zs/automatic-btn'
...
const Prompt: FC<ISimplePromptInput> = ({
  ...
  const minHeight = initEditorHeight || 175
  const [editorHeight, setEditorHeight] = useState(minHeight)

  return (
    <div className={cn((!readonly || gradientBorder) ? 'bg-[#F3F4F7] rounded-lg' : 'bg-gray-50', ' relative shadow-md')}>
      <div className='rounded-xl bg-[#F3F4F7] px-[14px] pb-[14px] pt-[19px]'>
        {!noTitle && (
          <div className="flex justify-between items-center mb-[14px]">
            <div className="flex items-center space-x-1">
              <div className='text-[#212B36] text-base leading-4'>{mode !== AppType.completion ? t('appDebug.chatSubTitle') : t('appDebug.completionSubTitle')}</div>
              {/* {!readonly && (
                <Tooltip
        ...
        <PromptEditorHeightResizeWrap
          className='px-3 pt-3 min-h-[175px] bg-white rounded-lg text-sm text-[#212B36]'
          height={editorHeight}
          minHeight={minHeight}
